...
class test_testReadConfig(unittest.TestCase):
    def test_invalidPathName(self):
        self.assertRaises(ValueError,femtoconfig.Config().readConfig,'test')

class test_pvconfigmatch(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.tconf = femtoconfig.Config()
        cls.tconf.readConfig("configs/s20_coherent1.json")
        cls.tconf.expandPVs()
        cls.refconfname = "VITARA1"
    ...
    def test_dither_level_match_CONDITIONAL(self):
        if self.refconf.use_dither:
            self.assertEqual(self.tconf.dither_level,self.refconf.dither_level)
        else:
            pass
